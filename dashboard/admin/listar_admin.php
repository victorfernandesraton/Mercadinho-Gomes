...
if ($tipo == "nome") {
    ...
    if (empty($_POST['busca'])) {
        $query_1 = 'SELECT id,nome,dadd FROM admin '.$order.';';
        $stmt_1 = $conect->prepare($query_1);
        $stmt_1->execute();
        ...
        $texto = '"%'.$_POST['busca'].'%"'; 
        $query_1 = 'SELECT nome,id,dadd FROM admin WHERE nome LIKE '.$texto.' '.$order.';';
        ...
        if (empty($_POST['busca'])) {
            ...
            $texto = '"%'.$_POST['busca'].'%"'; 
            $query_1 = 'SELECT nome,id,dadd FROM admin WHERE id LIKE '.$texto.' '.$order.';';
            $stmt_1 = $conect->prepare($query_1);
            $stmt_1->execute();
